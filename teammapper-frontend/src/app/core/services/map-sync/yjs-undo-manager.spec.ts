...
describe('canUndo/canRedo reactive state', () => {
  ...
  it('enables canUndo after edit', () => {
    ...
    expect({ canUndo, canRedo }).toEqual({ canUndo: true, canRedo: false });
  });
  ...
  it('enables canRedo after undo', () => {
    ...
    expect({ canUndo, canRedo }).toEqual({ canUndo: false, canRedo: true });
  });
...
describe('undo/redo operations', () => {
  ...
  beforeEach(() => {
    ({ doc, nodesMap } = createYjsContext());
    // Pre-populate root (not tracked by undo)
    doc.transact(() => {
  ...
  it('undo removes created node, redo restores it with properties', () => {
    ...
    expect(yMapToNodeProps(nodesMap.get('n1')!)).toEqual(
      expect.objectContaining({ id: 'n1', name: 'Test', coordinates: coords })
    );
  });
  ...
  it('undo restores deleted node at original coordinates', () => {
    ...
    expect(yMapToNodeProps(nodesMap.get('n1')!).coordinates).toEqual(coords);
  });
