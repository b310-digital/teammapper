...
test('toggles dark mode in settings and persists', async ({ page }) => {
  await page.goto('/');
  await page.getByText('Create mind map').click();
  ...
  // Wait for dark mode toggle to be visible
  const darkModeToggle = page.locator('.dark-mode-row mat-slide-toggle');
  ...
  // Navigate back to map editor
  await page.locator('.close-button').click();
  await expect(page.locator('.map')).toBeVisible();

  // Body should retain dark-mode state in the editor
  if (initiallyDark) {
    await expect(page.locator('body')).not.toHaveClass(/dark-mode/);
  } else {
    await expect(page.locator('body')).toHaveClass(/dark-mode/);
