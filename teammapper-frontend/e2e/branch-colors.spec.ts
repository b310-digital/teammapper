...
test('branch colors are inherited correctly from parent nodes', async ({ page }) => {
  await page.goto('/');
  ...
  const firstBranchColor = await branches[0].evaluate(el => 
    window.getComputedStyle(el).fill
  );
  const secondBranchColor = await branches[1].evaluate(el => 
    window.getComputedStyle(el).fill
  );
  ...
  const childOfFirstColor = await allBranches[2].evaluate(el => 
    window.getComputedStyle(el).fill
  );
  const childOfSecondColor = await allBranches[3].evaluate(el => 
    window.getComputedStyle(el).fill
  );
  ...
  const anotherChildColor = await finalBranches[4].evaluate(el => 
    window.getComputedStyle(el).fill
  );
