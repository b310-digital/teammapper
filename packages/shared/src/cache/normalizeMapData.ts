import { ClientMap, MapNode, MapNodeColors, MapNodeCoordinates, MapNodeFont, MapNodeImage, MapNodeLink, MapOptions } from '../models';
...
export const normalizeMapData = (rawMap: unknown): ClientMap => {
  ...
  const rawOptions = typeof r.options === 'object' && r.options !== null ? (r.options as Record<string, unknown>) : {};
  const options: MapOptions = {};
  if (typeof rawOptions.fontMaxSize === 'number' && !isNaN(rawOptions.fontMaxSize)) options.fontMaxSize = rawOptions.fontMaxSize;
  if (typeof rawOptions.fontMinSize === 'number' && !isNaN(rawOptions.fontMinSize)) options.fontMinSize = rawOptions.fontMinSize;
  if (typeof rawOptions.fontIncrement === 'number' && !isNaN(rawOptions.fontIncrement)) options.fontIncrement = rawOptions.fontIncrement;
  ...
  if (r.createdAt !== undefined) result.createdAt = r.createdAt as Date | number | string;
  if (r.lastModified !== undefined) result.lastModified = r.lastModified as Date | number | string;
  if (r.lastAccessed !== undefined) result.lastAccessed = r.lastAccessed as Date | number | string;
  if (typeof r.deleteAfterDays === 'number') result.deleteAfterDays = r.deleteAfterDays;
  if (r.deletedAt !== undefined) result.deletedAt = r.deletedAt as Date | number | string;
