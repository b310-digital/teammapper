...
- **Backend MapsService** (`maps.service.ts`): Simplified. Per-operation validation logic (constraint error mapping, parent validation for real-time ops) largely removed. New persistence method added for Y.Doc decode-and-save.
- **Dependencies**: Add `yjs`, `y-websocket`, `y-protocols`. Remove `socket.io`, `socket.io-client` (for data ops). Remove `cache-manager` usage for client tracking (Awareness replaces it).
- **REST API**: Unchanged. Map CRUD, export/import endpoints continue to work against the same DB tables.
- **MMP library**: Unchanged internally. MapSyncService adapter layer handles the integration.
- **Auth/EditGuard**: Shift from per-message guard to WebSocket connection-level authentication.
- **Map import flow**: Changes from disconnect-all/replace/reconnect to a Y.Doc transaction (clear + repopulate), synced automatically to all clients.
- **Map deletion flow**: Moves from Socket.io broadcast to REST endpoint + Y.Doc destruction + client WS disconnect detection.
- **E2E tests**: Will need updates since the transport layer changes entirely.
